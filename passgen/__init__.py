...
def exists():
    ...
    check50.exists("password_generator.py")

@check50.check(exists)
def test_valid_input():
    """Valid input produces correct output."""
    check = check50.run("python password_generator.py").stdin("8").stdin("2").stdin("2")
...
def test_non_integer_input():
    ...
    check = check50.run("python password_generator.py").stdin("abc").stdin("def").stdin("ghi")
...
def test_zero_minimum_length():
    ...
    check = check50.run("python password_generator.py").stdin("0").stdin("1").stdin("1")
